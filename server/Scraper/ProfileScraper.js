...
const scrapeProfile = async (url) => {
  ...
  const data = await page.evaluate(() => {
    ...
    const experienceElements = document.querySelectorAll('.pvs-entity__position-group-role-item');
    const experiences = Array.from(experienceElements).slice(0, 3).map(el =>
      el.innerText.split('\n')[0] || 'N/A'
    );

    // Skills: Top 3
    const skillElements = document.querySelectorAll('.pvs-entity__skill-category');
    const skills = Array.from(skillElements).slice(0, 3).map(el =>
      el.innerText.split('\n')[0] || 'N/A'
    );
